...
from Model.model import  FC_EF
from Model.unetPlusPlus4 import unetPlusPlus
...
def define_D(input_nc, ndf, netD,
    ...
    if netD == 'basic':
        ...
        net = CD_Discriminator(input_nc,ndf,n_layers=3,norm_layer=norm_layer)
    else:
...
class CD_Discriminator(nn.Module):
    def __init__(self, input_nc,ndf=256,n_layers=3,norm_layer=nn.BatchNorm2d):
        super(CD_Discriminator, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
       ...
       # self.Cd_model = FC_EF(6).to(device, dtype=torch.float)
        self.Cd_model = unetPlusPlus(6,2).to(device, dtype=torch.float)

        kw = 4
        padw = 1
        sequence = [
            nn.Conv2d(512, ndf, kernel_size=kw, stride=2, padding=padw),
            nn.LeakyReLU(0.2, True)
        ]
    ...
    def forward(self, input):
        h4,cd_logit = self.Cd_model(input)
        Dcm = self.model(h4)
        return cd_logit,Dcm
