...
class DoubleConvolutionLayer(nn.Module):
    ...
    def __init__(self, n_channels_input, n_channels_output):
        ...
        self.double_layer = nn.Sequential(
                                          AtrousSeparableConvolution(in_channels=n_channels_input,out_channels=n_channels_output,kernel_size=3,stride=1, padding=1, dilation=1, bias=True),
                                          #nn.Conv2d(n_channels_input, n_channels_output, kernel_size=3, padding=1),
                                          nn.BatchNorm2d(n_channels_output),
                                          nn.ReLU(inplace=True),
                                          nn.Dropout2d(p=0.05),
                                          AtrousSeparableConvolution(in_channels=n_channels_output, out_channels=n_channels_output,kernel_size=3,stride=1, padding=1, dilation=1, bias=True),
