...
from Model.basicUnet import Downscaling_layer, ExpandingLayer, DoubleConvolutionLayer, FinalLayer, Bottleneck
...
class unetPlusPlus(nn.Module):
    ...
    def __init__(self, n_channels, n_classes):
       ...
       # filter_sizes = [64, 8, 8, 8, 8]
        ...
        self.layer_D1 = DoubleConvolutionLayer(filter_sizes[4],filter_sizes[4])
        self.layer_D2 = DoubleConvolutionLayer(filter_sizes[4], filter_sizes[4])
        ...
        self.final3 = FinalLayer(filter_sizes[2], 64, n_classes)
        self.final4 = FinalLayer(filter_sizes[1], 32, n_classes)
        self.final5 = FinalLayer(filter_sizes[0]+filter_sizes[1], n_classes, n_classes)
    def forward(self, input):
        ...
        D_logit1 = self.layer_D1(x4_0)
        D_logit2 = self.layer_D2(D_logit1)

        x0_1 = self.layer0_1(self.multiple_cat([self.upsampling(x1_0),x0_0]))#FEM是一个multiple cat 加upsampling?
        ...
        x_final = self.multiple_cat([x0_4, x0_2, x0_3])
        output5 = self.final5(x_final)



        return D_logit2,output5
       # return  output5
